# 2.0 Управление порядком выполнения программы
## Псевдо-код
Представим - злой начальник дал нам очередное задание. Ему нужна программа, которая будет говорить, является ли некоторое число четным или нет.
Поразмыслив, вы в голове придумывайте примерно такой алгоритм
```
    считать число
    если число четное
        вывести "да"
    иначе
        вывести "нет"
```
Такое представление алгоритма еще называют псевдо-кодом, потому что мы описываем алгоритм конструкциями, похожими на конструкции языка программирования. Но стоит заметить, что псевдо-код может являться намного менее формальным, и в нем могут присутсвовать предложения вроде
```
    // Здесь приведен алгоритм нахождения человека в адресной книжке
    открыть книжку посередине
    если фамилия человека на этой странице
        мы нашли его
    иначе если его фамилия правее
        искать его в правой части
    иначе если его фамилия левее
        искать его в левой части
    иначе
        человека нету в данной книжке
```
Понятно, что слова вроде **искать его в правой части** никак не объясняют, как мы собираемся реализовывать этот поиск - они лишь объясняют основную идею. Этим и хорош псевдо-код - мы можем описывать суть алгоритмов, не вдаваясь в подробности их реализации на каком-то конкретном языке программирования.

## Если, иначе
Итак, как же нам реализовать наш первый псевдокод в программе?
В языке С слову **если** соответсвует ключевое слово **if**, а слову **иначе**  - **else**.
Например, чтобы выполнить какой-то кусок программы только, если число a больше нуля, нужно написать
```c
if(a > 0){
    // выполнить если а больше нуля
} else {
    // выполнить иначе, т.е. если а не больше нуля (a <= 0)
}
```
А как же проверить, что введенное число четное? Для этого нужно немного вспомнить математику. Четное число при делении на 2 всегда дает остаток 0, а нечетное 1.
Как мы уже знаем, остаток от деления находится операцией %.
Таким образом наш первый псевдо-код можно реализовать так
```c
#include <stdio.h>

int main(){
    int number;

    printf("Введите ваше число\n"); // Небольшая подсказка пользователю
    scanf("%d", &number);

    if(number % 2 == 0){
        printf("Число %d четное!\n", number);
    } else {
        printf("Число %d нечетное!\n", number);
    }
}
```
Подумайте (или испытайте), всегда ли работает наша программа? Будет ли она работать если пользователь введет 0? Отрицательное число? Что если пользователь введет букву?

## Множественные условия

Для примера напишем решим еще одну простую задачу. Нужно по введенным координатам точки определить, в какой из 4 частей плоскости лежит точка.
Для начала набросаем основной алгоритм 
```
пользователь вводит два числа, х и у
если х > 0 и у > 0
    вывести "Первая плоскость"
иначе
    если x < 0 и у > 0
        вывести "Вторая плоскость"
    иначе
        если x < 0 и у < 0
            вывести "Третья плоскость"
        иначе
            если x > 0 и y < 0
                вывести "Четвертая плоскость"
```
Как видите, конструкции, которые мы уже знаем, позволяют нам описать решение, однако сам текст программы выглядит не очень элегантно. Последняя строчка имеет аж 4 отступа, что часто говорит о нехорошо написанном коде. Если вы посмотрите на него внимательно, вы заметите часто повторяющуюся конструкцию 
```
иначе
    если
```
Действительно, ее можно заменить просто на 
```
иначе если
```
Перепишем наш код, используя новую конструкцию
```
пользователь вводит два числа, х и у
если х > 0 и у > 0
    вывести "Первая четверть"
иначе если x < 0 и у > 0
    вывести "Вторая четверть"
иначе если x < 0 и у < 0
    вывести "Третья четверть"
иначе если x > 0 и y < 0
    вывести "Четвертая четверть"
```
Наш код теперь выглядит компактнее и проще читается. Теперь переведем наш псевдо-код в программу С
```c
#include <stdio.h>
int main(void){
    int x, y;
    scanf("%d %d", &x, &y);

    if(x > 0 && y > 0){
        printf("Первая четверть\n");
    } else if(x < 0 && y > 0){
        printf("Вторая четверть\n");
    } else if(x < 0 && y < 0){
        printf("Третья четверть\n");
    } else if(x > 0 && y < 0){
        printf("Четвертая четверть\n");
    }

    return 0;
}
```
Как видите, **если** и **иначе** так и остаются **if** и **else**.

## Логические операторы

В предыдущем тексте вы заметили знак **&&**, который стоит на месте **и**. Дейсвительно, двойной амперсант означает **Логическое И**. Это один из операторов булевой алгебры. Его значение равно правде, только если оба его операнда равны правде (то есть правда && правда = правда, в остальном случае ложь). Хорошим примером для него может служить рецепт приготовления блюда - для омлета вам понадобятся яйца **и** молоко. Если хотя бы одного из компонентов нету, омлета вы уже не получите.

В противовес **и** существует оператор **или*. Его значение равно правде, если хотя бы один из его операндом равен правде. Примером могут служить какие-нибудь желания - хочу научиться играть на гитаре **или** съездить в отпуск **или** новую NVIDIA Quadro FX 5800. Вы будете счастливы, выполнись хотя бы одно из этих условий, однако выполнение двух или более условий также оставят вас счастливыми. Подробнее про булевую алгебру можно почитать [тут](http://electrik.info/main/fakty/229-buleva-algebra-chast-1-nemnogo-istorii.html).

Возвращаясь к нашей проблеме, что означет эта запись?
```c
if(x > 0 && y > 0){
    // код
}
```
Это означает, что код будет выполняться только, если х > 0 **и** y > 0. Если хотя бы одно из этих условий не верно (x <= 0, y <= 0), код выполняться не будет.

Выполните нашу программу и вводите в нее разные координаты. Всегда ли вы получаете нужный ответ?

## Неполнота решения
Внимательный читатель мог заметить, что мы не учли некоторые случаи. Что будет, если пользователь в одной из координат задаст 0? Программа сломается? Или выведет какой-то ответ? Или ничего не выведет и завершится? Проверьте это сами, если уже не сделали (но сначала попробуйте ответить устно).

Исправить эту ошибку мы можем, определяя, лежит ли точка на одной из осей (т.е. равна ли одна из координат 0). Однако мы займемся этим в **Главе 3 Функции**, поскольку это потребует небольшого усложнения кода.

## Еще про логические операторы

Представим такую программу - пользователь вводит код продукта, который он хочет купить, и ему выдается его цена. Сейчас в нашем магазине около ста товаров, и все они стоят по 100 рублей (очень стабильный магазин). Однако недавно на склад завезли итальянский сыр, который стоит аж 120 рублей. Попробуем обновить нашу программу. Раньше она выводила 100 для любого из товаров, однако теперь для сыра нам нужно вывести 120. Как это сделать существующими средствами?

```
// Коды всех товаров
...
коробка_конфет_код = 100
итальянский_сыр_код = 101
...
если код_товара == 1 или код_товара == 2 или код_товара == 3...(и так до 100)
    вывести 100
если код_товара == 101 
    вывести 120    
```

Видно, что наш код получается слишком громоздким. Заметьте, нам нужно вывести 100 для всех товаров, котороые **не** итальянский сыр.
```
если код_товара не равен итальянский_сыр_код
    вывести 100
иначе
    вывести 120    
```
Слова **не равен** в языке С обозначаются как **!=**. Знак **!** означает отрицание любого действия или значения. Перепишем наш псевдо-код на С
```c
if(user_input_code != italian_cheese_code){
    printf("%d", 100);
} else {
    printf("%d", 120);
}
```
Приведем еще пару примеров использования оператора **!**
```c
if(! user_input==italian_cheese_code) // Тоже самое, что и user_input != italian_cheese_code
if(a == b && c != d)
if(! (a == b && c == d)) // В чем различия этой строчки и строчки выше?
```

## Контрольные вопросы

1. Какие конструкции в языке С используются для условного исполнения кода?
2. Зачем нужен оператор else? Всегда ли его нужно применять? Приведите пример, когда его использование обязательно и когда не нужно совсем.
3. Что такое логические операции? Приведите пример их использования в реальной жизни.
4. Какие логические операции вы знаете?
5. Зачем нужен оператор отрицания **!**? Можно ли обойтись совсем без него?
6. Оператор **и** еще называют логическим умножением, а оператор **или** логическим сложением. Исходя из этого, подумайте, какой из этих двух операторов имеет более высокий приоритет?
7. Какие из операторов имеют более высокий приоритет - математические или логические? Другими словами, как выполнится следующий код ``` a + b < 5 && a - b > -10 ```? 

## Чтение на дом
Прочтите следующий код и объясните, что он делает

```c
int a, b;
scanf("%d %d", &a, &b);
if(a > b){
    printf("Greater\n");
} else if(a < b){
    printf("Lower\n");
} else {
    printf("Equal\n");
}
```

```c
int money, currency;
printf("Введите количество денег и идентефикатор валюты\n");
printf("1 для долларов, 2 для евро\n");
scanf("%d %d", &money, &currency);

if(currency == 1){
    printf("%d рублей\n", money * 60);
} else if(currency == 2){
    printf("%d рублей\n", money * 70)
} else {
    printf("Неопознанная валюта")
}
```